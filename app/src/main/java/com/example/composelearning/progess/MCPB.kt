...
fun CircularProgressIndicator(
    ...
    val sweepPerColor = 360f / (determinateColors.size + 1) // Include green

    // Animate the rotation for the indeterminate part
    ...
    // Calculate the base offset for all arcs
    val baseOffset = StartAngleOffset + (currentRotation.value * RotationAngleOffset) % 360f

    // Define animation for the indeterminate green arc sweep
    val indeterminateSweep = transition.animateFloat(
        0f,
        sweepPerColor, // Minimum sweep for green
        infiniteRepeatable(
            animation = tween(
                durationMillis = RotationDuration,
                easing = CircularEasing // Example easing for oscillation
            )
        )
    )

    // Calculate offsets and sweeps for determinate arcs (excluding green)
    val determinateOffsets = determinateColors.mapIndexed { index, _ ->
        (baseOffset + (index + 1) * sweepPerColor) % 360f
    }
    val determinateSweeps = determinateColors.map { _ -> sweepPerColor }
    ...
    Canvas(
        ...
        // Draw the indeterminate green arc first
        drawArc(
            color = Color.Gray, // Green color
            startAngle = 0f,
            sweepAngle = indeterminateSweep.value,
            useCenter = false,
        ...
        // Draw the determinate arcs
        determinateColors.forEachIndexed { index, color ->
            val offset = determinateOffsets[index]
            val sweep = determinateSweeps[index]
            drawArc(
                color = color,
                startAngle = baseOffset,
                sweepAngle = sweep,
                useCenter = false,
                topLeft = Offset(stroke.width / 2, stroke.width / 2),
                size = Size(size.width - stroke.width, size.width - stroke.width),
...
//    )
//
//    val greenCurrentRoation = transition.animateValue(
//        0,
//        RotationsPerCycle,
//        Int.VectorConverter,
//        infiniteRepeatable(
...
//
//        val greenCurrentRotationAngleOffset = (greenCurrentRoation.value + RotationAngleOffset) % 360
//
...
//        val greenOffset = StartAngleOffset + greenCurrentRotationAngleOffset + baseRotation.value
//
//
//        val redSweepAngle = abs(endAngle.value - startAngle.value)
//        val greenSweepAngle = redSweepAngle / 2f
...
object ProgressIndicatorDefaults {
    ...
    val CircularStrokeWidth: Dp = 8.dp
...
/*@VisibleForTesting*/
internal val CircularIndicatorDiameter = 24.dp
...
private const val RotationDuration = 1000
