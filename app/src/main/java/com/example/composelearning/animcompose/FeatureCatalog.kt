...
val FeatureCatalog: List<AnimationCategory> = listOf(
    ...
    AnimationCategory(
        ...
        AnimScreen.AnimationBasics, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Value-Based Animations",
        "Custom types and keyframes",
        AnimScreen.ValueBasedAnimations, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Transition Animations",
        "State-driven animations",
        AnimScreen.TransitionAnimations, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Physics Animations",
        "Spring, decay, fling",
        AnimScreen.PhysicsAnimations, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "April 2026 Updates",
        "New features: Morphing Shapes, PullToRefreshBox, Shared Elements",
        AnimScreen.April2026Features, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Animated Balance Counter",
        "Count-up balance animation from 0 to target",
        AnimScreen.AnimatedBalance, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Bouncing Ball",
        "Vertical bounce animation with screen boundaries",
        AnimScreen.BouncingBall, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Pulsating Circles",
        "Multiple overlapping pulse animations",
        AnimScreen.PulsatingCircles, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Circular Reveal",
        "Expanding circular path reveal (WhatsApp style)",
        AnimScreen.CircularReveal, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Staggered Grid Animation",
        "Grid items appearing with delayed entrance",
        AnimScreen.StaggeredGrid, FeatureGroup.FUNDAMENTALS,
    ),
    AnimationCategory(
        "Cell Loader Animation",
        "Cell fading animation",
        AnimScreen.SequentialFadeGridScreen, FeatureGroup.FUNDAMENTALS,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.ShaderDemos, FeatureGroup.SHADERS_IMAGES,
    ),
    AnimationCategory(
        "Image Processing (AGSL)",
        "Instagram-style filters rendered as an AGSL RuntimeShader RenderEffect.",
        AnimScreen.ImageProcessing, FeatureGroup.SHADERS_IMAGES,
    ),
    AnimationCategory(
        ...
        AnimScreen.RiveoPageCurl, FeatureGroup.SHADERS_IMAGES,
    ),
    AnimationCategory(
        "Blur Effects",
        "Modifier.blur and Haze frosted-glass demos",
        AnimScreen.BlurEffects, FeatureGroup.SHADERS_IMAGES,
    ),
    AnimationCategory(
        "Zoomable Image",
        "Pinch-to-zoom and pan with rememberTransformableState — double-tap to reset.",
        AnimScreen.ZoomableImage, FeatureGroup.SHADERS_IMAGES,
    ),
    AnimationCategory(
        "Overlapping Images",
        "Custom Layout that stacks avatars with a slider-controlled overlap factor",
        AnimScreen.OverlappingImages, FeatureGroup.SHADERS_IMAGES,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.ClearTodo, FeatureGroup.GESTURES_TOUCH,
    ),
    AnimationCategory(
        "Stacked Tinder Cards",
        "Swipeable cards with interaction physics",
        AnimScreen.StackedCards, FeatureGroup.GESTURES_TOUCH,
    ),
    AnimationCategory(
        "Physics Game",
        "Bubble Pop Game",
        AnimScreen.GameEnvironment, FeatureGroup.GESTURES_TOUCH,
    ),
    AnimationCategory(
        "Draggable Side Sheet",
        "Panel that pulls out from the right side of the screen",
        AnimScreen.DraggableSheet, FeatureGroup.GESTURES_TOUCH,
    ),
    AnimationCategory(
        "Time Range Knob",
        "Circular 24h dial with two draggable knobs — drag to set bedtime and wake-up.",
        AnimScreen.TimeRangeKnob, FeatureGroup.GESTURES_TOUCH,
    ),
    AnimationCategory(
        "Sensor Reactive Card",
        "Credit card that tilts based on device sensors",
        AnimScreen.SensorCard, FeatureGroup.GESTURES_TOUCH,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.ArcCarousel, FeatureGroup.LISTS_LAYOUTS_PAGERS,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.TabsSample, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Product Shared Elements",
        "Cinema-style transitions between product list and details",
        AnimScreen.SharedElementProduct, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Chat App Navigation",
        "Shared element transitions in a messaging UI",
        AnimScreen.ChatApp, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Fluid Tab Bar",
        "Morphing indicators and spring-based interactions",
        AnimScreen.FluidTabs, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Circular Menu",
        "Animated menu buttons emerging from center",
        AnimScreen.CircleMenu, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Spotlight Walkthrough",
        "Interactive feature highlighting overlay system",
        AnimScreen.Spotlight, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    AnimationCategory(
        "Tutorial Overlay",
        "Coach-mark walkthrough on a LazyColumn — auto-scrolls to off-screen targets.",
        AnimScreen.TutorialOverlay, FeatureGroup.NAVIGATION_TRANSITIONS,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.SmoothProgress, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Multi-Color Progress",
        "Comet-style indeterminate circular progress bar",
        AnimScreen.MultiColorProgress, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Gradient Progress Bar",
        "Arc-based progress with linear gradient",
        AnimScreen.GradientProgress, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Path Progress",
        "Animated path-drawing border button and progress bar",
        AnimScreen.PathProgress, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "File Delete animation",
        "File delete Animation",
        AnimScreen.FileDeleteAnimation, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Button Animation",
        "Spring-driven button press animation",
        AnimScreen.ButtonAnimation, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Strava Save Activity",
        "Pill button morphs to circular loader, then point-sample lerps into a checkmark.",
        AnimScreen.SaveActivity, FeatureGroup.PROGRESS_BUTTONS,
    ),
    AnimationCategory(
        "Google Calling Animation",
        "Mimics the Google Dialer calling animation with bouncing FAB and sliding/fading arrows.",
        AnimScreen.GoogleCalling, FeatureGroup.PROGRESS_BUTTONS,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.MarqueeDemo, FeatureGroup.TEXT_TYPOGRAPHY,
    ),
    AnimationCategory(
        "Text Shimmer Effects",
        "Premium shimmer, wave, and spotlight text animations",
        AnimScreen.TextShimmer, FeatureGroup.TEXT_TYPOGRAPHY,
    ),
    AnimationCategory(
        "Squiggly Spans Math",
        "Understanding animated squiggly underlines via Sine waves",
        AnimScreen.SquigglySpans, FeatureGroup.TEXT_TYPOGRAPHY,
    ),
    AnimationCategory(
        "Squiggly Slider",
        "Material Expressive style slider with a wavy animated progress track.",
        AnimScreen.SquigglySlider, FeatureGroup.TEXT_TYPOGRAPHY,
    ),
    ...
    AnimationCategory(
        ...
        AnimScreen.ProtobufDemo, FeatureGroup.APP_CLONES,
    ),
)
